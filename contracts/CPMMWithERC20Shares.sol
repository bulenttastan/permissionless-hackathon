...
contract CPMMWithERC20Shares is Ownable {
    ...
    function buyYes(uint256 sharesOut) external {
        uint256 amountIn = getSharesToDollar(
            sharesOut,
            yesUsdcReserve,
            yesTokenReserve
        );
        ...
        yesToken.transfer(msg.sender, sharesOut);
        yesTokenReserve -= sharesOut;
    ...
    function getYesProbability() public view returns (uint256) {
        return yesUsdcReserve * 10000 / (yesUsdcReserve + noUsdcReserve);
    }

    function getNoProbability() public view returns (uint256) {
        return noUsdcReserve * 10000 / (yesUsdcReserve + noUsdcReserve);
    }
